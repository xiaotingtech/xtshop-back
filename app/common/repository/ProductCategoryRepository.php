...
{
    //ProductCategory验证器
    protected $productCategoryValidate;
    //ProductCategory的model
    protected $productCategoryModel;

    public function __construct()
    {
        $this->productCategoryValidate = sunnier_validate('app\common\validate\ProductCategory');
    ...
    public function info($id){
        return $this->productCategoryModel->where('id',$id)->find();
    }
    /**
     * @param $data
     * @return array
    ...
    public function save($data){
        if (!$this->productCategoryValidate->scene('save')->check($data)) {
            return ['status'=>0, 'msg'=>$this->productCategoryValidate->getError()];
        }
        if(!empty($data['icon'])){
        ...
        if(!empty($data['id'])){
            ...
            if(!$this->productCategoryModel->update($data)){
                return [
                    'status'=>0,
                    'msg'=>'保存失败!',
                ];
            ...
            if(!$this->productCategoryModel->save($data)){
                return [
                    'status'=>0,
                    'msg'=>'保存失败!',
                ];
    ...
    public function getList($where,$page=1,$listRow=10){
        $count=$this->productCategoryModel->where($where)
            ->count();
        if($count>0) {
            $list = $this->productCategoryModel
                ->field('id,name,pid as parentId,status,show_navigation,product_num,product_unit,sort_num,level')
                ->where($where)
    ...
    public function getTreeList($where){
        $count=$this->productCategoryModel->where($where)->count();
        if($count>0) {
            $list = $this->productCategoryModel
     ...
     * @throws \think\db\exception\DbException
     * @throws \think\db\exception\ModelNotFoundException
     * Description:获取推荐的分类列表，首页用
     * Author:sunnier
     * Email:xiaoyao_xiao@126.com
     * 2020-10-20 16:32
     */
    public function getRecommendList($where='',$sort='sort_num ASC'){
        $count=$this->productCategoryModel->where($where)
            ->where('pid','>',0)
            ->where('show_home',1)->count();
        if($count>0) {
            $list = $this->productCategoryModel
                ->field('id,name,icon,pid')
                ->where($where)
     ...
     * 2020-10-20 17:00
     */
    public function getActivityList($where='',$sort='sort_num DESC,id DESC'){
        $count=$this->productCategoryModel->where($where)
            ->where('pid','>',0)
